...
class QuadrupedAgent:
    ...
    def update_from_rollout(self, last_value):
        ...
        logits = self.actor_model(states_tensor)
        dist = torch.distributions.Categorical(logits=logits)
        log_probs = dist.log_prob(actions_tensor).sum(-1)
        entropy = dist.entropy().sum(-1)
        state_values = self.critic_model(states_tensor).squeeze(-1)

        actor_loss = -(log_probs * advantages_tensor.detach()).mean() - self.entropy_coeff * entropy.mean()
        critic_loss = F.mse_loss(state_values, returns_tensor.detach())
        ...
        metrics = {
            ...
            "total_loss": float(actor_loss.item() + self.critic_loss_coeff * critic_loss.item()),
